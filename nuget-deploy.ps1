...
if($IsMacOS) {
    ...
    $LinuxPackage = "nuget-packages/CloudVeil.proxy-native-macos.$version.nupkg"
}
...
if($IsMacOS) {
    ...
    Copy-Item -Path $LinuxPackage -Destination $destinationPath
}
...
if($deployNuget[0] -eq 'y') {
    ...
    if($IsMacOS) {
	    ...
	    dotnet nuget push $LinuxPackage -k $apiKey -s https://api.nuget.org/v3/index.json
    }
}
